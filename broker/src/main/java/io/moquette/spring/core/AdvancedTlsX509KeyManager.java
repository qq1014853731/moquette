 ...
 */
@Slf4j
public class AdvancedTlsX509KeyManager extends X509ExtendedKeyManager {
    ...
    public AdvancedTlsX509KeyManager(KeyStore ks, char[] password)
        throws KeyStoreException,
        NoSuchAlgorithmException, UnrecoverableKeyException {

        credentialsMap = new HashMap<>();
        serverAliasCache = Collections.synchronizedMap(new HashMap<>());
        updateIdentityCredentials(ks, password);
    }
     ...
     * @param password 密码
     */
    public void updateIdentityCredentials(KeyStore ks, char[] password) throws KeyStoreException, NoSuchAlgorithmException, UnrecoverableKeyException {
        if (ks == null) {
            return;
        }

        synchronized (serverAliasCache) {
            synchronized (credentialsMap) {
                ...
                for (Enumeration<String> aliases = ks.aliases();
                    ...
                    log.debug("***");
                    log.debug("found key for : {}", alias);
                    for (int i = 0; i < certs.length; i++) {
                        log.debug("chain [{}] = {}", i, certs[i]);
                    }
                    log.debug("***");
                }
            }
            serverAliasCache.clear();
        }
     ...
     * @return
     */
    public X509Credentials updateIdentityCredential(String alias, PrivateKey key, X509Certificate[] certs) {
        synchronized (credentialsMap) {
            return credentialsMap.put(alias, new X509Credentials(key, certs));
        }
    }
    ...
    private String[] getAliases(String keyType, Principal[] issuers) {
        ...
        for (Map.Entry<String, X509Credentials> entry :
            ...
            if (issuers.length == 0) {
                ...
                aliases.add(alias);
                log.debug("matching alias: {}", alias);
            } else {
